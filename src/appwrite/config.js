    ...
    async getAllBookmarksByUser(userId) {
        try {
            const response = await this.databases.listDocuments(
                ...
                conf.appwriteBookmarksCollectionId,
                [
                    Query.equal('userId', userId) // Filter by userId only
                ]
            );
    
            // Check if the response contains documents and return the full list of bookmarks
            if (response.documents && Array.isArray(response.documents) && response.documents.length > 0) {
                return response.documents; // Return all matching documents (bookmarks)
            } else {
